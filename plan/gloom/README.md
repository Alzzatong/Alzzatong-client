...
- 진행할 일 : vercel 배포하기
  - yarn build 명령어로 사전에 나오는 에러 다 잡기
  - 기본적으로 vercel로 배포하려면 build시 에러가 없어야한다.
...
- 진행할 일 : 매칭탭 구현
- 무한렌더링 문제를 해결
  - useEffect에 대한 이해도 부족으로 일어난 현상

<br></br>

### 2023.11.11

- 진행할 일 : 매칭탭 구인처 매칭 부분 1차 완료
- 미팅관리 조회 UI 구성중
...
- 문제 발생 : 상위 컴포넌트가 바뀌어서 렌더링 될 때마다, 하위 컴포넌트에는 영향이 없는 문제.
- value와 defaultvalue
- 진행한 것(작업): 매칭 탭의 검색 박스들 컴포넌트화 (components/Matching/SelectBox)
  - 매칭 구인처 박스 -> CompanyBox
  - 매칭 구인직무 박스 -> RecruitBox
  - 매칭 구인직무 박스안에 구인상태들 -> EmploymentStatusTextBox
- 진행할 일
...
- 진행한 것(회의) : solji님이랑 merge 및 진행사항 공유
  - solji : 조회기능 구현 못함(), 등록기능에서 추가하기 버튼 이슈(인덱스 이슈가 있음)
  - gloom : Icon 파일 위치 이동, Template 변경이슈, 매칭파트 구현중
- (회사)구인직무와 (시니어)희망 직무의 통일성 필요! 매칭 기능에서 조회시 필요!
  - 회사 위치관련된 정보는 dropbox로 통일가능!
- onChange -> onBlur() 변경
- 라이브러리 추가 : 모달, 캘린더
- 진행한 것(작업) : 구인처, 구인직무를 기준으로, 시니어의 희망 직무와 비교하여, 겹치는 부분만 조회하여 시니어정보 리스트 가져오기
  - 그리고 조회페이지처럼 띄워주기
- 진행한 것(작업) : 구인직무 컴포넌트화 -> 시니어 조회할때 재사용되기 때문에

<br></br>

### 2023.11.08

- 해야할 것 : 모달창 띄우고, 저장기능 잘되는지 확인하기
- **문제 발생** : 등록 폼 입력하다가 엔터누르면 저장되게됨.
  - 빈칸이 있으면 저장되기 전까진 KeyboardEvent로 막아두기!
  - 각각 input마다 제한사항 넣어주기
  - [참고링크](./learning/JavaScript.md)

<br></br>

### 2023.11.07

- 해야할 것 : 직종코드 검색하여, 입력 기능 만들기
- **문제 발생** : Select drop down 기능을 하는데, 첫번째 값은 선택이 안되는 이슈 발생
  - 해결방안, 아무것도 없는 기본값 추가해줌
  - { id: "직무코드", title: "선택해주세요." }, 이로써 첫번째값을 선택할 일이 없어진다.
- 해야할 것 : 직종코드를 모달창으로 만들어서 처리하기
  - 직종 코드 입력칸 클릭시 모달창이 띄워지게 처리
  - 모달창 외 클릭 시, 모달창 닫히게 진행

<br></br>

### 2023.11.06

- **문제 발생** : 저장할때 입력값과 조회할 때 출력값의 데이터 타입이 약간씩 다른 문제가 발생
...
- **해야할 것** : `구인처 등록`저장할 때, (회사정보 저장 후, 구인 정보 저장함)중간에 실패했다면 전부 롤백하는 기능 필요!

<br></br>

### 2023.11.05

- **문제 발생** : phoneInput에 데이터가 바로 들어가지 않는 현상
...
- 절차
  ...
  - 국세청\_사업자등록정보 진위확인 및 상태조회 서비스 오픈API 활용신청 (승인이 자동단계라 내용만 잘 적으면 됨)
